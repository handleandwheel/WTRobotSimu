...
IMUNode::IMUNode(ros::NodeHandle &nh)
{
	...
	inertialUnitSub = nh.subscribe<sensor_msgs::Imu>(model_name + "/imu_ang_pos/quaternion", 1,
													 &IMUNode::imu_ang_pos_callback, this);
	accelerometerSub = nh.subscribe<sensor_msgs::Imu>(model_name + "/imu_lin_acc/values", 1,
													  &IMUNode::imu_lin_acc_callback, this);
	gyroSub = nh.subscribe<sensor_msgs::Imu>(model_name + "/imu_ang_vel/values", 1, &IMUNode::imu_ang_vel_callback,
											 this);
	imuPub = nh.advertise<sensor_msgs::Imu>("/WTRobotSimu/imu", 1);
	
	ros::Timer controlThread = nh.createTimer(ros::Duration(imuNodeConfig.sampling_time_period * 1e-3),
											  &IMUNode::timer_callback, this);
...
void IMUNode::enable(ros::ServiceClient &basicTimeStepClient, ros::ServiceClient &enableClient, int &sampling_period)
{
	...
	if (sampling_period % int(basic_time_step) == 0)
	{
		...
		if (sampling_period_srv.response.success)
			ROS_INFO("[IMU_NODE_INFO][part:%d] Sensor enabled", sensor_part);
		else
		{
			ROS_WARN("[IMU_NODE_INFO][part:%d] Enable failed. Trying again...", sensor_part);
			for (int i = 0; i < 20; i++)
			{
				if (sampling_period_srv.response.success)
				{
				...
				if(i = 19) ROS_ERROR("[IMU_NODE_INFO][part:%d] Enable failed! Please restart the node!", sensor_part);
			}
		}
	}
	else if (basic_time_step <= 1)
	{
		...
		enableClient.call(sampling_period_srv);
		if (sampling_period_srv.response.success)
			ROS_WARN("[IMU_NODE_INFO][part:%d] Invalid sampling time. Sampling time is set to 1ms", sensor_part);
		else
		{
			ROS_WARN("[IMU_NODE_INFO][part:%d] Enable failed. Trying again...", sensor_part);
			for (int i = 0; i < 20; i++)
	...
	else
	{
		...
		enableClient.call(sampling_period_srv);
		if (sampling_period_srv.response.success)
			ROS_WARN("[IMU_NODE_INFO][part:%d] Invalid sampling time. Sampling time is set to robot basic time step:%d.",
					 sensor_part, int(basic_time_step));
		else
		{
			ROS_WARN("[IMU_NODE_INFO][part:%d] Enable failed. Trying again...", sensor_part);
			for (int i = 0; i < 20; i++)
			{
				...
				{
					ROS_WARN(
							"[IMU_NODE_INFO][part:%d] Invalid sampling time. Sampling time is set to robot basic time step:%d.",
							sensor_part, int(basic_time_step));
					break;
				}
				ROS_ERROR("[IMU_NODE_INFO][part:%d] Enable failed! Please restart the node!", sensor_part);
			}
		}
	}
	sensor_part++;
