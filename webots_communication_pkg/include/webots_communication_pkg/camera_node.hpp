...
CameraNode::CameraNode(ros::NodeHandle &nh)
{
	...
	enable(robotBasicTimeStepClient, cameraEnableClient, cameraNodeConfig.samplingPeriod);
...
void CameraNode::img_callback(const sensor_msgs::Image::ConstPtr &msg)
{
	...
	if (info_srv.response.width / cameraNodeConfig.resolution[0] >
		info_srv.response.height / cameraNodeConfig.resolution[1])
	{
	...
	cv::resize(cv_img_ptr->image, cv_img_ptr->image,
			   cv::Size(cameraNodeConfig.resolution[0], cameraNodeConfig.resolution[1]), cv::INTER_LINEAR);
	imgPub.publish(cv_img_ptr->toImageMsg());
}

void CameraNode::enable(ros::ServiceClient &basicTimeStepClient, ros::ServiceClient &enableClient, int &sampling_period)
{
	...
	if (sampling_period % int(basic_time_step) == 0)
	{
		...
		if (sampling_period_srv.response.success)
			ROS_INFO("[CAMERA_NODE_INFO] Sensor enabled.");
		else
		{
			ROS_WARN("[CAMERA_NODE_INFO] Enable failed. Trying again...");
			for (int i = 0; i < 20; i++)
			{
				if (sampling_period_srv.response.success)
				{
				...
				if(i = 19) ROS_ERROR("[CAMERA_NODE_INFO] Enable failed. Please restart the node");
			}
		}
	}
	else if (basic_time_step <= 1)
	{
		...
		enableClient.call(sampling_period_srv);
		if (sampling_period_srv.response.success)
			ROS_WARN("[CAMERA_NODE_INFO] Invalid sampling time. Sampling time is set to 1ms");
		else
		{
			ROS_WARN("[CAMERA_NODE_INFO] Enable failed. Trying again...");
			for (int i = 0; i < 20; i++)
	...
	else
	{
		...
		enableClient.call(sampling_period_srv);
		if (sampling_period_srv.response.success)
			ROS_WARN("[CAMERA_NODE_INFO] Invalid sampling time. Sampling time is set to robot basic time step:%d.",
					 int(basic_time_step));
		else
		{
			ROS_WARN("[CAMERA_NODE_INFO] Enable failed. Trying again...");
			for (int i = 0; i < 20; i++)
			{
				...
				{
					ROS_WARN(
							"[CAMERA_NODE_INFO] Invalid sampling time. Sampling time is set to robot basic time step:%d.",
							int(basic_time_step));
					break;
				}
				if(i = 19) ROS_ERROR("[CAMERA_NODE_INFO] Enable failed. Please restart the node");
